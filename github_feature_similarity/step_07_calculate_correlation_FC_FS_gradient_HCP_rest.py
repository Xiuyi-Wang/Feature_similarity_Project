...
# calculate correlation between FC gradient 1 and FS gradient 2
path_FS = '/home/xiuyi/Data/HCP/99_Results/hctsa_timeseries_fs/gradient_HCP_rest_FS/kernel_None_embedding_dm'

path_FC = '/home/xiuyi/Data/HCP/11_FC_xDF_z_mean_group/kernel_None_embedding_dm'
...
for i in range(1,4):
    file_FC_gradient = os.path.join(path_FC,'HCP_rest_FC_gradient_%s_kernel-None_embedding-dm_flip.ptseries.nii'%(i))
    data_FC_grad = nib.load(file_FC_gradient).get_fdata()[0]
    ...
    for j in range(1,4):
        ...
        r, p = stats.pearsonr(data_FC_grad,data_FS_grad)
        print (i, j ,r, p)

        for hemi in ['L','R']:

            fig_file = os.path.join(path_output,'Correlation_FC_grad_%s_FS_grad_%s_%sH.png'%(i,j,hemi))
            if hemi == 'L':
                # load parcellated structural neuroimaging maps
                data_FC_grad_half = data_FC_grad[0:200]
                data_FS_grad_half = data_FS_grad[0:200]
                df_pconn = df_pconn_L
            elif hemi =='R':
                # load parcellated structural neuroimaging maps
                data_FC_grad_half = data_FC_grad[200::]
