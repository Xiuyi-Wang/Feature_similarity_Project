...
from function_edit_scene_figures import add_colorbar2
import shutil
...
path_base_group = '/home/xiuyi/Data/HCP/99_Results/hctsa_timeseries_fs/gradient_HCP_rest_FS'
...
path_output_group = '%s/kernel_%s_embedding_%s_jet'%(path_base_group ,kernel, embedding)
os.makedirs(path_output_group,exist_ok=True)

file_fs_group = '/home/xiuyi/Data/HCP/99_Results/hctsa_timeseries_fs/rest_fs_similarity_z.xlsx'

fs_matrix_group = pd.read_excel(file_fs_group)
...
# gradient_setting_group = GradientMaps(n_components=comp_num, approach=embedding, kernel=kernel)
gradient_group = gradient_setting_group.fit(fs_matrix_group.values)

fig_scree = join(path_output_group, '%s_FS_gradient_scree_kernel-%s_embedding-%s.png' % (task,  kernel, embedding))
fig, ax = plt.subplots(1, figsize=(5, 4))
ax.scatter(range(1,gradient_setting_group.lambdas_.size+1), gradient_setting_group.lambdas_)
ax.set_title('%s_FS_grad_scree_kernel-%s_embedding-%s' % (task,  kernel, embedding))
...
for comp_id in range(comp_num):
    ...
    # save each gradient
    file_map = join(path_output_group, '%s_FS_gradient_%s_kernel-%s_embedding-%s.ptseries.nii' % (task, comp_id + 1, kernel, embedding))
    file_map_flip = join(path_output_group, '%s_FS_gradient_%s_kernel-%s_embedding-%s_flip.ptseries.nii' % (task, comp_id + 1, kernel, embedding))
    fig = join(path_output_group, '%s_FS_gradient_%s_kernel-%s_embedding-%s.png' % (task, comp_id + 1, kernel, embedding))
    fig_flip = join(path_output_group, '%s_FS_gradient_%s_kernel-%s_embedding-%s_flip.png' % (task, comp_id + 1, kernel, embedding))
    ...
    if not os.path.exists(fig):
        title = '%s_FS_gradient_%s_kernel-%s_embedding-%s' % (task, comp_id + 1, kernel, embedding)
    ...
    if not os.path.exists(fig_flip):
        title = '%s_FS_gradient_%s_kernel-%s_embedding-%s_flip' % (task, comp_id + 1, kernel, embedding)
